...
export function Skyline({ theme }) {
  ...
  // Animate elements slightly
  useFrame((state) => {
    if (cloudsRef.current) {
      cloudsRef.current.position.x = Math.sin(state.clock.getElapsedTime() * 0.05) * 5
    }
  })

  // Theme configuration for building materials
          ...
          {idx % 2 === 0 && (
            <mesh position={[0, 0, b.d / 2 + 0.01]}>
              ...
              <meshStandardMaterial 
                ...
                emissiveIntensity={theme === 'day' ? 0.2 : 1.5}
                transparent
                opacity={0.8}
              />
            </mesh>
