...
export function FrontendWorkstation({ position, rotation = [0, 0, 0], onMonitorClick }) {
  ...
  return (
    <group position={position} rotation={rotation} {...monitorProps}>
      {/* Table Top */}
...
export function BackendWorkstation({ position, rotation = [0, 0, 0], onMonitorClick }) {
  const monitorProps = onMonitorClick ? {
    onClick: (e) => {
      e.stopPropagation()
      onMonitorClick()
...
export function FullStackWorkstation({ position, rotation = [0, 0, 0], onMonitorClick }) {
  const monitorProps = onMonitorClick ? {
    onClick: (e) => {
      e.stopPropagation()
      onMonitorClick()
  ...
  return (
    <group position={position} rotation={rotation} {...monitorProps}>
      ...
      {/* Dual Monitors (One Frontend layout, one code compiler/git graph) */}
      {/* Screen 1 (UI view) */}
      ...
      {/* Screen 2 (Git Branch Flow / Docker) */}
      <group position={[0.34, 0.75, -0.18]} rotation={[0, -0.15, 0]}>
        <mesh position={[0, 0.1, -0.04]} castShadow>
          <cylinderGeometry args={[0.015, 0.015, 0.2, 8]} />
          <meshStandardMaterial color="#475569" metalness={0.8} />
...
export function ArchitectureWhiteboard({ position, rotation = [0, 0, 0] }) {
  return (
    <group position={position} rotation={rotation}>
      ...
      {/* System Flow Diagram representation */}
      <group position={[0, -0.15, 0.03]}>
        ...
        {/* Connections links representations */}
...
// Sleek Office Cabin Dividers (Solid base, Glass top, Metallic frames)
