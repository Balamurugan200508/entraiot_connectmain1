...
export function StylizedCharacter({ position, rotation = [0, 0, 0], type = 'typing', shirtColor = '#2563eb', hairColor = '#78350f', skinColor = '#fbcfe8' }) {
  ...
  useFrame((state) => {
    ...
    if (type === 'typing') {
      leftArmRef.current.rotation.x = -Math.PI / 4.5 + Math.sin(t * 18) * 0.08
      leftArmRef.current.rotation.y = Math.sin(t * 9) * 0.04
      rightArmRef.current.rotation.x = -Math.PI / 4.5 + Math.cos(t * 16) * 0.08
      rightArmRef.current.rotation.y = -Math.cos(t * 8) * 0.04
      headRef.current.rotation.x = Math.sin(t * 2.2) * 0.04
      headRef.current.rotation.y = Math.cos(t * 1.6) * 0.02
    } else if (type === 'debugging') {
      // Scratching head animation, thinking pose
      leftArmRef.current.rotation.x = -Math.PI / 6 + Math.sin(t * 1.5) * 0.05
      
      // Right hand raises to scratch head
      rightArmRef.current.rotation.x = -Math.PI / 1.3 + Math.sin(t * 4) * 0.1
      rightArmRef.current.rotation.y = -Math.PI / 6 + Math.cos(t * 3) * 0.05
      
      headRef.current.rotation.x = 0.15 + Math.sin(t * 1.2) * 0.06
      ...
      headRef.current.rotation.y = Math.sin(t * 1.5) * 0.25
    } else if (type === 'standing') {
      headRef.current.rotation.x = Math.sin(t * 1.5) * 0.02
      leftArmRef.current.rotation.z = Math.sin(t * 1.2) * 0.03 + 0.05
      rightArmRef.current.rotation.z = -Math.sin(t * 1.2) * 0.03 - 0.05
    }
  })
